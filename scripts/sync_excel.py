...
for t in teams:
    ...
    # Generate Direct Payment Proof Link (via Edge Function to avoid Excel length limits)
    receipt_path = t.get('payment_receipt_path')
    payment_link = ""
    if receipt_path:
        payment_link = f"{SUPABASE_URL}/functions/v1/view-receipt?path={urllib.parse.quote(receipt_path)}"
