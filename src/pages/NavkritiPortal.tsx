...
export default function NavkritiPortal() {
    ...
    if (session) {
      const fetchTeamData = async () => {
        ...
        try {
          ...
          const psDraft = data.psDraft;
          if (psDraft) {
            setProblemStatement(prev => prev || psDraft.problem_statement || '');
            setPsTitle(prev => prev || psDraft.ps_title || '');
            setCategory(prev => prev || psDraft.category || '');
            setDomain(prev => prev || psDraft.domain || '');
            setSolutionTitle(prev => prev || psDraft.solution_title || '');
            setOrganization(prev => prev || psDraft.organization || '');
          }
        } catch (err: any) {
          setDataError(err.message || 'Failed to fetch team data');
        } finally {
  ...
  const isDraftAfterClose = now > new Date(navkritiConfig.psDrafting.closes).getTime();
  ...
  const handleDraftSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: functionError } = await supabase.functions.invoke('save-ps-draft', {
        body: { problem_statement: problemStatement, ps_title: psTitle, category, domain, solution_title: solutionTitle, organization },
        headers: { Authorization: `Bearer ${session.token}` }
      });
      if (functionError) {
      ...
      toast.success('PS Draft saved successfully!');
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 bg-slate-50 dark:bg-slate-950 px-4 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-sm mt-8 mb-8">
          <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
            <Layout className="w-6 h-6 text-blue-500" /> Phase 1: Problem Statement Drafting
          </h2>
          
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-xl p-6 mb-8 text-sm text-amber-800 dark:text-amber-300">
            <h3 className="font-bold flex items-center gap-2 mb-2"><AlertCircle className="w-5 h-5" /> Important Instructions</h3>
            <ul className="list-disc pl-5 space-y-2">
              <li>This drafting form closes at <strong>{navkritiConfig.psDrafting.displayDeadline}</strong>. Whatever is saved here at that time will be auto-submitted as your final choice.</li>
              <li>Before selecting a Problem Statement, you MUST check the official SIH Portal to ensure your chosen PS has not reached the 500-team submission limit.</li>
              <li>You are expected to carry forward this exact same PS for the main SIH competition.</li>
            </ul>
          </div>
          ...
          {draftSuccess && (
            <div className="mb-6 bg-green-50 dark:bg-green-900/20 border-l-4 border-green-500 p-4 rounded-r flex items-start gap-3 text-sm">
              ...
              <span className="text-green-800 dark:text-green-300">Problem Statement Draft saved successfully!</span>
            </div>
          )}
          ...
          <form onSubmit={handleDraftSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  type="text"
                  required
                  disabled={isDraftAfterClose}
                  value={problemStatement}
                  onChange={(e) => setProblemStatement(e.target.value)}
                  placeholder="e.g., SIH26041"
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none disabled:opacity-50"
                />
              </div>
              <div>
            ...
            <button
              type="submit"
              disabled={isDrafting || isDraftAfterClose}
              className="w-full py-4 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-bold transition-all disabled:opacity-50"
            >
              {isDraftAfterClose ? 'Drafting Closed (Auto-Submitted)' : isDrafting ? 'Saving Draft...' : 'Save Problem Statement Draft'}
            </button>
          </form>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-sm">
          <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
            <Upload className="w-6 h-6 text-blue-500" /> Phase 2: Project Submission
          </h2>
            ...
            <>
              <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/50 rounded-xl p-6 mb-8 text-sm text-blue-800 dark:text-blue-300">
                <ul className="list-disc pl-5 space-y-1">
                  ...
                  <li>This will use the Problem Statement details you drafted above.</li>
